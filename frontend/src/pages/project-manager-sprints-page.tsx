...
export function ProjectManagerSprintsPage() {
  ...
  return (
    <section className="rounded-md border border-border/80 bg-card/95 p-4 shadow-sm sm:p-5">
      <div className="flex flex-wrap items-end justify-between gap-3 pb-4">
        ...
        />
        <Button asChild>
          <Link to={`/project-manager/projects/${project.id}/sprints/new`}>Create Sprint</Link>
        </Button>
      </div>
        ...
        <TableBody>
          {sprints.map((sprint) => (
            <TableRow
              ...
              onClick={() => navigate(`/project-manager/projects/${project.id}/sprints/${sprint.id}`)}
            >
              <TableCell>
